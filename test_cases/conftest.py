...
from utilities.event_listner import EventListener
...
def init_web_driver(request):
    if get_data('Excute_Applitools').lower() == 'yes':
...
def get_desktop_driver():
    ...
    driver = appium.webdriver.Remote(get_data('WinAppDrive_Service'), dc)
    return driver
...
def get_firefox():
    ...
    fire_fox_driver = selenium.webdriver.Firefox(service=srv)
    return fire_fox_driver
...
# catch exception and error
def pytest_exception_interact(node, call, report):
    if report.failed:
        if globals()['driver'] is not None:  # if it is None -> this is exception from API test
            image = get_data('ScreenshotPath') + '_screen_' + str(get_time_stamp()) + '.png'
